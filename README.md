...
A human adds public job URLs to a queue (a Google Sheet, or a local Excel file). At 7:00am, or on demand with `python -m applyapp run`, ApplyApp processes **new and stuck rows**: fetch the posting, compare it to your seed documents, write a truthful resume and cover letter, run a critic pass, and save the files for you to proofread and submit. Each model step can use Claude or any OpenAI-compatible API. Finished files are Google Docs, or formatted Word documents in a local folder.
...
Job queue (Sheet or jobs.xlsx)
  ...
  ingest pending rows
        │
        ├─► fetch posting text
        ├─► load seed docs (Drive folder or local folder)
        ▼
     analyze          ┐
...
| Seed documents | Accomplishments, writing samples, ATS guidance. Google Drive, or a local folder. |
...
Files in the seed folder (Drive, or `LOCAL_SEED_DIR`, including subfolders) are classified by name and used differently:

| Seed | How the agent uses it |
| --- | --- |
...
5. Create a Sheet for the job queue and two Drive folders (seeds, output), unless those are local. Paste their URLs or IDs into `.env`.
6. Put seed documents in the seed folder (Drive, or `LOCAL_SEED_DIR`).
...
`DOCUMENT_STORE=google` reads the Drive seed folder and writes Google Docs. `DOCUMENT_STORE=local` reads `LOCAL_SEED_DIR` and writes formatted `.docx` files into `LOCAL_OUTPUT_DIR` (Calibri, navy, 0.75 in margins, the same block styles as the Docs). File names and folder layout still decide each seed's role.
...
| --- | --- | --- |
| `analyze` | `ANALYZE_` | Posting vs seed documents |
...
| --- | --- |
| `python -m applyapp init` | Create `~/ApplyApp` with example seeds, an empty queue, and a local `.env` |
